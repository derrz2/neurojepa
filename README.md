...
**Private preparation release:** the 10.42M encoder is included and verified.
The target 2.65M checkpoint is still missing; its configuration is included,
but requesting its weights raises an explicit error. The definitive mapping
from these candidates to the paper's EFLOPs table remains to be confirmed.

## Quick start

Use Python 3.10 or later. The release was tested with Python 3.10,
PyTorch 2.4.1, and timm 1.0.22. After downloading or cloning this repository,
...
| --- | ---: | ---: | --- |
| `2m` | 2,653,568 | 200 (target) | Configuration only; target weights missing |
...
missing weights, checksum rejection, and downstream initialization. Missing
2m weights are tested as an error condition, not counted as a successful
checkpoint load. See [verification scope](docs/VERIFICATION.md).
